...
- **Orçamentos** — monta com materiais cadastrados (preço × quantidade, aceitando **fração: 0,25 / 0,5 / 0,75 metro**) + mão de obra **editável item a item**; **desconto em R$, em % ou arredondando para o valor final**, sempre mostrando quanto foi abatido; **fotos das notas de compra anexadas ao PDF**; totais automáticos; **gera PDF profissional** na hora, até sem internet; ao gerar, **pergunta se quer abrir o PDF na tela**; lista geral com filtro por status
- **Ciclo de venda completo** — rascunho → enviado → aprovado/recusado; aprovar gera cobrança automaticamente, e o **recebimento (total ou parcial) pode ser registrado direto na tela do orçamento aprovado** ou pelo atalho na lista de orçamentos
...
- **Materiais** — catálogo **em ordem alfabética**, com busca, filtro por categoria, edição e exclusão
...
- **Alarme no relógio do celular** — no app Android, botão no compromisso cria um **alarme de verdade no app Relógio** (30 min antes), que toca mesmo no silencioso e tem o snooze do próprio Relógio; válido para compromissos nas próximas 24 h (limite da API do Android), com a notificação exata cobrindo o resto
...
| `orcamentos` | clienteId, status, data | itens, fotos e desconto persistidos junto — total sempre revalidado |
...
- [x] **F9** — orçamento completo: mão de obra editável, quantidade fracionada (0,25 m), desconto/arredondamento com o abatimento explícito, fotos das notas anexadas ao PDF, materiais em ordem alfabética, recebimento (total ou parcial) direto no orçamento aprovado, e alarme no app Relógio do aparelho
- [ ] tema nativo dos seletores de data/hora (cores do app no diálogo do Android)
- [ ] alarme de tela cheia dentro do app (Rota A do [plano de alarme](docs/PLANO-ALARME-ANDROID.md)), se o Relógio do sistema não bastar
